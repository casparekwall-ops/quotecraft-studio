  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [{ data: quote }, { data: lineItems }, { data: custList }] = await Promise.all([
        supabase
          .from("quotes")
          .select("id, quote_number, status, issue_date, expiry_date, subtotal, tax, discount, total, notes, terms, customer_id")
          .eq("id", id)
          .maybeSingle(),
        supabase
          .from("quote_items")
  ...
  return (
    <AppLayout>
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-foreground">Edit {quoteNumber}</h1>
          <p className="text-muted-foreground">Update quote details and line items.</p>
        </div>
      </div>
