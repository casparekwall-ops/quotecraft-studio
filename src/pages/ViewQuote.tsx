...
import { ArrowLeft, ArrowRightLeft, Send, Pencil, CheckCircle2, XCircle, Eye } from "lucide-react";
...
interface QuoteDetail {
  id: string; quote_number: string; status: string; issue_date: string;
  expiry_date: string | null; subtotal: number; tax: number; discount: number;
  total: number; notes: string | null; terms: string | null;
  customer_id: string | null; currency: string;
...
const ViewQuote = () => {
  ...
  const fetchQuote = async () => {
    ...
    const { data } = await supabase
      .from("quotes")
      .select("id, quote_number, status, issue_date, expiry_date, subtotal, tax, discount, total, notes, terms, customer_id, currency, customers(name, email)")
      .eq("id", id).maybeSingle();
    setQuote(data as any);
    const { data: lineItems } = await supabase.from("quote_items").select("*").eq("quote_id", id).order("id");
  ...
  const cur = (quote?.currency || "USD") as CurrencyCode;

  const updateStatus = async (status: string, label: string) => {
    if (!id) return;
    const { error } = await supabase.from("quotes").update({ status }).eq("id", id);
    if (error) { toast.error(error.message); } else { toast.success(`${t.quotes.markedAs} ${t.status[label as keyof typeof t.status] || label}`); fetchQuote(); }
  };
  ...
  const convertToInvoice = async () => {
    ...
    const { data: invoice, error } = await supabase.from("invoices").insert({
      ...
      total: quote.total, notes: quote.notes, quote_id: quote.id, currency: quote.currency,
    }).select().single();
  ...
  if (!quote) return <AppLayout><div className="text-center py-12"><p className="text-muted-foreground">{t.quotes.quoteNotFound}</p><Button variant="outline" className="mt-4" onClick={() => navigate("/quotes")}>{t.quotes.backToQuotes}</Button></div></AppLayout>;

  const previewData = {
    type: "quote" as const, number: quote.quote_number, status: quote.status,
    issueDate: quote.issue_date, expiryDate: quote.expiry_date || undefined,
    ...
    <AppLayout>
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <DocumentPreview data={previewData} defaultTemplate={settings.default_quote_template}
            trigger={<Button variant="outline" size="sm"><Eye className="mr-1 h-3.5 w-3.5" />{t.quotes.preview}</Button>} />
          {quote.status === "draft" && (
            <Button variant="outline" size="sm" onClick={() => updateStatus("sent", "sent")}><Send className="mr-1 h-3.5 w-3.5" />{t.quotes.sendQuote}</Button>
          )}
          {(quote.status === "sent" || quote.status === "draft") && (
            <Button variant="outline" size="sm" onClick={() => updateStatus("accepted", "accepted")}><CheckCircle2 className="mr-1 h-3.5 w-3.5" />{t.quotes.markAccepted}</Button>
          )}
          {(quote.status === "sent" || quote.status === "draft") && (
            <Button variant="outline" size="sm" onClick={() => updateStatus("rejected", "rejected")}><XCircle className="mr-1 h-3.5 w-3.5" />{t.quotes.markRejected}</Button>
          )}
          {quote.status === "accepted" && (
            <Button size="sm" onClick={convertToInvoice}><ArrowRightLeft className="mr-1 h-3.5 w-3.5" />{t.quotes.convertToInvoice}</Button>
          )}
        </div>
      </div>
        ...
        <div className="space-y-6 lg:col-span-2">
          <div className="rounded-xl border border-border bg-card p-6 shadow-card">
            ...
            <div className="grid gap-4 sm:grid-cols-2 text-sm">
              ...
              <div><span className="text-muted-foreground">{t.quotes.currency}</span><p className="font-medium text-foreground">{quote.currency}</p></div>
            </div>
          </div>
        ...
        <div>
          <div className="sticky top-6 rounded-xl border border-border bg-card p-6 shadow-card">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-muted-foreground">{t.quotes.subtotal}</span><span className="text-foreground">{formatCurrency(Number(quote.subtotal), cur)}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">{t.quotes.tax}</span><span className="text-foreground">{formatCurrency(Number(quote.tax), cur)}</span></div>
              {quote.discount > 0 && (
                <div className="flex justify-between"><span className="text-muted-foreground">{t.quotes.discount}</span><span className="text-destructive">-{formatCurrency(Number(quote.discount), cur)}</span></div>
              )}
              <div className="flex justify-between border-t border-border pt-2 text-lg font-bold">
                <span className="text-foreground">{t.quotes.total}</span>
                <span className="text-foreground">{formatCurrency(Number(quote.total), cur)}</span>
              </div>
            </div>
          </div>
        </div>
