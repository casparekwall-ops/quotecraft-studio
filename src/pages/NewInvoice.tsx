...
const NewInvoice = () => {
  ...
  const handleSave = async (status: "draft" | "sent" = "draft") => {
    ...
    const invoiceItems = items.filter((i) => i.name.trim()).map((i) => ({
      invoice_id: invoice.id, item_name: i.name, description: i.description || null,
    ...
    setSaving(false);
    toast.success(status === "sent" ? t.invoices.saveAndSend : t.invoices.saveDraft);
    navigate("/invoices");
  };
    ...
    <AppLayout>
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground">{t.invoices.createNewInvoice}</p>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          ...
          <div className="rounded-xl border border-border bg-card p-6 shadow-card">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="font-semibold text-foreground">{t.invoices.lineItems}</h2>
              <Button variant="outline" size="sm" onClick={addItem}><Plus className="mr-1 h-3.5 w-3.5" />{t.invoices.addItem}</Button>
            </div>
              ...
              {items.map((item, idx) => (
                <div key={item.id} className="rounded-lg border border-border bg-muted/30 p-4">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <Input placeholder={t.invoices.itemName} value={item.name} onChange={(e) => updateItem(item.id, "name", e.target.value)} />
                    <Input placeholder={t.invoices.description} value={item.description} onChange={(e) => updateItem(item.id, "description", e.target.value)} />
          ...
          <div className="rounded-xl border border-border bg-card p-6 shadow-card">
            <h2 className="mb-4 font-semibold text-foreground">{t.invoices.notes}</h2>
            <Textarea placeholder={t.invoices.additionalNotes} value={notes} onChange={(e) => setNotes(e.target.value)} />
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="sticky top-6 rounded-xl border border-border bg-card p-6 shadow-card">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <Label>{t.invoices.template}</Label>
              ...
              <div className="flex items-center justify-between"><Label>{t.invoices.tax}</Label><Switch checked={taxEnabled} onCheckedChange={setTaxEnabled} /></div>
              {taxEnabled && (
                <div className="space-y-2"><Label>{t.invoices.taxRate}</Label><Input type="number" min={0} max={100} value={taxRate} onChange={(e) => setTaxRate(Number(e.target.value))} /></div>
              )}
              ...
              <div className="space-y-2 pt-2">
                <DocumentPreview
                  ...
                  trigger={<Button className="w-full" variant="outline"><Eye className="mr-1 h-3.5 w-3.5" />{t.invoices.preview}</Button>}
                />
                <Button className="w-full" variant="outline" onClick={() => handleSave("draft")} disabled={saving}>{saving ? t.invoices.saving : t.invoices.saveDraft}</Button>
                <Button className="w-full" onClick={() => handleSave("sent")} disabled={saving}>{saving ? t.invoices.saving : t.invoices.saveAndSend}</Button>
              </div>
            </div>
          </div>
        </div>
